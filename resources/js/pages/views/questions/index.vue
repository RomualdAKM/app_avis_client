...
const deleteQuestion =  (questionId) => {
  Swal.fire({
        ...
        if (result.value) {
            axios.get("/api/delete_question/" + questionId).then(() => {
                ...
                <div class="card-body">
                  <div class="overflow-auto custom-scroll"> 
                    <table class="table equal-space table-action invoice">
                      ...
                      <tbody> 
                        <tr class="hover:bg-gray-light dark:hover:bg-sidebar-dark2 invoice-item" v-for="question in questions" :key="question.id">
                         
                          <td class="min-w-[7.5rem]"> 
                            <h4 class="text-xs font-semibold text-title">{{ question.title }}</h4>
                          </td>
                         
                          
                          <td class="min-w-[3.125rem] text-right">
                            <div class="items-center gap-3 flex 2lg:hidden"> 
                              <router-link :to="{name: 'EditQuestion',params:{questionId:question.id}}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                </svg>
                             </router-link>
                             <button type="button" @click="deleteQuestion(question.id)">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
