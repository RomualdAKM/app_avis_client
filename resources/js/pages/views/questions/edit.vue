...
const getQuestion = async () => {
    let response = await axios.get(`/api/get_question/${questionId.value}`)
...
await axios.post("/api/edit_question/" + question.value.id, question.value)
.then((response) => {
    if(response.data.success){
        router.push("/questions")
        toast.fire({
        icon: "success",
        title: "Question Modifier avec success",
    })
    }else{
           toast.fire({
                  icon: "error",
    ...
    <div class="page-body"> 
          <div class="grid grid-cols-12 gap-4 add-product card">
            <div class="col-span-12 pr-4 border-r dark:border-r rtl:!border-r-0 rtl:!pr-4 dark:border-border-dark 2lg:col-span-12 2lg:border-b 2lg:pb-4 2lg:border-r-0 border-border-light rtl:border-l rtl:2lg:border-l-0  rtl:pl-4 ">
              <div> 
                <h5 class="text-title font-semibold mb-3">Modifier Question </h5>
                <form class="flex flex-col gap-4 m-0 sm:gap-3 theme-form">
                  ...
                  <!-- <div class="grid grid-cols-12 gap-5 sm:gap-3 sm:-mt-1"> -->
                    <div class="col-span-6 2md:col-span-6 sm:col-span-12">
                      ...
                      <input type="name" id="sku" v-model="question.title">
                    </div>
                  ...
                  <div class="flex flex-wrap gap-2 items-center justify-end">
                    ...
                    <a class="btn btn-primary py-1 text-white text-xs flex gap-2 items-center" href="#"><button @click="editQuestion()" type="button" >Modifier</button></a>
