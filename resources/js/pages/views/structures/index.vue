...
const getCompanies = () => {
    axios.get('/api/structures').then((response) => {
...
const deleteCompany =  (companyId) => {
  Swal.fire({
        ...
        if (result.value) {
            axios.get("/api/delete_structure/" + companyId).then(() => {
                Swal.fire("Delete", "company delete successfully", "success");
                ...
                <div class="card-body">
                  <div class="overflow-auto custom-scroll"> 
                    <table class="table equal-space table-action invoice">
                      ...
                      <tbody> 
                        <tr class="hover:bg-gray-light dark:hover:bg-sidebar-dark2 invoice-item" v-for="company in companies" :key="company.id">
                          <td class="min-w-[210px]">
                            <div class="flex items-center gap-2">
                              
                              <div class="flex items-center gap-[.5625rem]"> <img class="w-10 h-10 rounded-full" src="/assets/images/product/1.jpg" alt="Samsung TV">
                                <div><a href="invoice-preview.html">
                                    <h4 class="text-xs font-semibold text-title">{{ company.name }}</h4></a></div>
                              </div>
                            </div>
                          </td>
                          <td class="min-w-[7.5rem]"> 
                            <h4 class="text-xs font-semibold text-title">{{ company.email }}</h4>
                          </td>
                          <td class="min-w-[7.5rem]"> 
                            <h4 class="text-xs font-semibold text-content">{{ company.phone }}</h4>
                          </td>
                          <td class="min-w-[5.5rem]">
                            <h4 class="text-xs font-semibold capitalize text-warning">{{ company.ifu }}</h4>
                          </td>
                          <td class="min-w-[6.25rem]">
                            <h4 class="text-xs font-semibold text-title">{{ company.adresse }}</h4>
                          </td>
